...
clean: dist-clean
	rm --force --recursive .coverage .hypothesis/ .mypy_cache/ .pytest_cache/
	rm --force --recursive docs/_build/

# Remove code caches, or the entire virtual environment.
.PHONY: nuke-caches nuke
nuke-caches: clean
	find src/ -type d -name __pycache__ -exec rm --force --recursive {} +
	find tests/ -type d -name __pycache__ -exec rm --force --recursive {} +
nuke: nuke-caches
	if [ ! -z "${VIRTUAL_ENV}" ]; then \
